...
__global__ void ROIPoolForward(const int nthreads, const T* bottom_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    // Define an empty pooling region to be 0
    T maxval = is_empty ? 0 : -INFINITY;
    // If nothing is pooled, argmax = -1 causes nothing to be backprop'd
    int maxidx = -1;
    const T* bottom_data_this = bottom_data + roi_batch_ind * channels * height * width;
    ...
    if (argmax_data != nullptr)
      argmax_data[index] = static_cast<int>(maxidx);
  }
}

template <typename T>
__global__ void ROIPoolBackward(const int nthreads, const T* top_diff,
    ...
    for (int roi_n = 0; roi_n < num_rois; ++roi_n) 
    {
      ...
      for (int ph = phstart; ph < phend; ++ph) {
        for (int pw = pwstart; pw < pwend; ++pw) {
          int offset_index = static_cast<int>(offset_argmax_data[(ph * pooled_width + pw) * channels + c]);
          if (offset_index == (h * width + w) * channels + c) {
...
#define DECLARE_GPU_SPEC(T)                                                                     \
  template bool RoiPoolForwardGPU(const T* bottom_data, const float spatial_scale,              \
...
#define DECLARE_GPU_SPEC(T)                                                                                         \
  template bool RoiPoolBackwardGPU(const T* top_diff, const float spatial_scale,                                    \
